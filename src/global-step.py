...
"""
try:
    data = pd.read_csv('ASPENV12计算样本（共482个）.csv')
    print(f"成功加载数据，共 {len(data)} 条记录")
except FileNotFoundError:
    print("找不到ASPENV12计算样本（共482个）.csv文件")
    exit()
"""下面代码为分别绘制每个4D表征图的代码"""
# 定义特征和目标变量
features = ['T', 'P', 'D', 'L', 'Catalysis']
target = 'dms'

# 根据目标变量dms值分箱，用于颜色映射
dms_bins = pd.qcut(data[target], 4, labels=False)
colors = plt.cm.rainbow(dms_bins / max(dms_bins))
...
for i, combo in enumerate(feature_combinations):
    ...
    # 添加颜色条说明
    cbar = plt.colorbar(plt.cm.ScalarMappable(cmap='rainbow'), ax=ax)
